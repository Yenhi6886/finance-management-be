    ...
    private String generateHTMLContent(ReportDataResponse reportData) {
        ...
        if (!reportData.getCategoryStats().isEmpty()) {
            ...
            for (ReportDataResponse.CategoryReportData category : reportData.getCategoryStats()) {
                html.append("<tr>");
                html.append("<td>").append(category.getCategoryName()).append("</td>");
                html.append("<td>").append(category.getType().toString()).append("</td>");
                html.append("<td class='amount'>").append(formatCurrency(category.getTotalAmount())).append("</td>");
                html.append("<td>").append(category.getTransactionCount()).append("</td>");
                html.append("<td>").append(String.format("%.2f", category.getPercentage())).append("%</td>");
                html.append("</tr>");
            }
        ...
        if (!reportData.getWalletStats().isEmpty()) {
            html.append("<div class='section'>");
            html.append("<h2>Thống Kê Theo Ví</h2>");
            html.append("<table class='data-table'>");
            html.append("<thead><tr>");
            html.append("<th>Ví</th>");
            html.append("<th>Thu Nhập</th>");
            html.append("<th>Chi Tiêu</th>");
            html.append("<th>Số Dư Ròng</th>");
            html.append("<th>Số GD</th>");
            html.append("<th>Số Dư Hiện Tại</th>");
            html.append("</tr></thead>");
            html.append("<tbody>");
            
            for (ReportDataResponse.WalletReportData wallet : reportData.getWalletStats()) {
                html.append("<tr>");
                html.append("<td>").append(wallet.getWalletName()).append("</td>");
                html.append("<td class='amount income'>").append(formatCurrency(wallet.getTotalIncome())).append("</td>");
                html.append("<td class='amount expense'>").append(formatCurrency(wallet.getTotalExpense())).append("</td>");
                html.append("<td class='amount ").append(wallet.getNetAmount().compareTo(java.math.BigDecimal.ZERO) >= 0 ? "income" : "expense").append("'>")
                   .append(formatCurrency(wallet.getNetAmount())).append("</td>");
                html.append("<td>").append(wallet.getTransactionCount()).append("</td>");
                html.append("<td class='amount'>").append(formatCurrency(wallet.getCurrentBalance())).append("</td>");
                html.append("</tr>");
            }
        ...
        // Transaction details (first 50 transactions to avoid too long PDF)
        List<ReportDataResponse.TransactionReportData> transactions = reportData.getTransactions();
        if (!transactions.isEmpty()) {
            html.append("<div class='section'>");
            html.append("<h2>Chi Tiết Giao Dịch (50 giao dịch gần nhất)</h2>");
            html.append("<table class='data-table'>");
            html.append("<thead><tr>");
            ...
            int limit = Math.min(50, transactions.size());
